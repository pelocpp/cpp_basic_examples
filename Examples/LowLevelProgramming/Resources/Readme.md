...
| Funktion `clearBit` | `void clearBit(unsigned int value, int pos);`<br/>Löscht in der Variablen `value` das Bit an Position `pos`. |
| Funktion `isBitSet` | `bool isBitSet(unsigned int value, int pos);`<br/>Liefert `true` zurück, wenn in der Variablen `value` das Bit an Position `pos` gesetzt ist, andernfalls `false`. |
| Funktion `isBitCleared` | `bool isBitCleared(unsigned int value, int pos);`<br/>Liefert `true` zurück, wenn in der Variablen `value` das Bit an Position `pos` nicht gesetzt ist, andernfalls `false`. |
